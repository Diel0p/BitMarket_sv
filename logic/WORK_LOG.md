...
- Replaced with in-memory dict storage for demo reliability
- Decision rationale: eliminates external dependency without changing any service or controller code
...
| In-memory DB | MongoDB | Zero setup for demo, trivially swappable |
...
- Data is lost on server restart (in-memory DB)
...
git commit -m "feat(db): replace MongoDB with in-memory store for demo reliability"
